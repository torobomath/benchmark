...
%            Number of atoms       :   70 (   5 equality;  23 variable)
%            Maximal formula depth :   14 (  13 average)
%            Number of connectives :   58 (   1   ~;   0   |;   1   &;  55   @)
%                                         (   0 <=>;   1  =>;   0  <=;   0 <~>)
%                                         (   0  ~|;   0  ~&)
...
thf(p2_qustion,question,
    ...
    @ ^ [V_f: 'R2R'] :
        ( ! [V_x_dot_0: $real] :
            ( ( 'funapp/2' @ V_f @ ( $sum @ V_x_dot_0 @ 1.0 ) )
            = ( $sum @ ( $quotient @ 1.0 @ 2.0 ) @ ( 'sqrt/1' @ ( $difference @ ( 'funapp/2' @ V_f @ V_x_dot_0 ) @ ( '^/2' @ ( 'funapp/2' @ V_f @ V_x_dot_0 ) @ 2.0 ) ) ) ) )
        & ~ ( ? [V_y: $real] :
              ! [V_x: $real] :
                ( ( 'funapp/2' @ V_f @ V_x )
                = V_y ) ) ) )).
...
thf(p2_answer,answer,(
    ^ [V_f_dot_0: 'R2R'] :
      ...
      = ( 'fun/1'
        @ ^ [V_x_dot_1: $real] :
            ( $quotient @ ( $sum @ 1.0 @ ( 'abs/1' @ ( 'cos/1' @ ( $quotient @ ( $product @ 'Pi/0' @ V_x_dot_1 ) @ 2.0 ) ) ) ) @ 2.0 ) ) ) ),
