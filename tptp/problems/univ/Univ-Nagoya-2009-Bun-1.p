...
%            Number of atoms       :  262 (  27 equality;  55 variable)
%            Maximal formula depth :   21 (  14 average)
%            Number of connectives :  204 (   0   ~;   0   |;  27   &; 177   @)
%                                         (   0 <=>;   0  =>;   0  <=;   0 <~>)
%                                         (   0  ~|;   0  ~&)
...
thf(p1_qustion,question,
    ( 'find/1' @ ( 'ListOf' @ $real )
    @ ^ [V_ab: ( 'ListOf' @ $real )] :
      ? [V_a: $real,V_b: $real,V_A: '3d.Point',V_B: '3d.Point',V_C: '3d.Point'] :
        ( ( V_ab
          = ( 'cons/2' @ $real @ V_a @ ( 'cons/2' @ $real @ V_b @ ( 'nil/0' @ $real ) ) ) )
        & ( ( '3d.vec/2' @ '3d.origin/0' @ V_A )
          = ( '3d.vec3d/3' @ 1.0 @ 0.0 @ 0.0 ) )
        & ( ( '3d.vec/2' @ '3d.origin/0' @ V_B )
          = ( '3d.vec3d/3' @ V_a @ V_b @ 0.0 ) )
...
thf(p1_answer,answer,(
    ^ [V_ab_dot_0: ( 'ListOf' @ $real )] :
      ( V_ab_dot_0
      = ( 'cons/2' @ $real @ ( $quotient @ 1.0 @ 3.0 ) @ ( 'cons/2' @ $real @ ( $quotient @ ( $product @ 2.0 @ ( 'sqrt/1' @ 2.0 ) ) @ 3.0 ) @ ( 'nil/0' @ $real ) ) ) ) ),
    answer_to(p1_question,[])).

thf(p2_answer,answer,(
    ^ [V_S_dot_0: $real] :
      ( V_S_dot_0
      = ( $quotient @ ( 'sqrt/1' @ 2.0 ) @ 3.0 ) ) ),
    answer_to(p2_question,[])).

thf(p3_answer,answer,(
    ^ [V_V_dot_0: $real] :
      ( V_V_dot_0
      = ( $quotient @ ( 'sqrt/1' @ 2.0 ) @ 18.0 ) ) ),
