...
(def-answer p (PLam R_max (exists (T S)
          (&& (= T (foot-of-perpendicular-line-from-to (point Qx Qy Qz) (xy-plane)))
              (||
               (&& (! (= T (point Px Py 0)))
             (point-symmetry S (point Px Py 0) T)
             (on R_max (line S T))
             (= (rad-of-angle (angle S Q R)) (/ (Pi) 2))
             )
               (&& (= T (point Px Py 0))
             (on R_max (circle (point Px Py 0) (distance (point Qx Qy Qz) (point Px Py 0)) (vec3d 0 0 1)))
          )
               )
           )
          )))
